...
function ArbolBinarioEncuesta(){
  ...
  ArbolBinarioEncuesta.prototype.insert=function(parent, newValue){
    if(parent.value.Grado==newValue.value.Grado){
  ...
  ArbolBinarioEncuesta.prototype.add=function(value){
  ...
  ArbolBinarioEncuesta.prototype.balancear=function(parent){
  ...
  ArbolBinarioEncuesta.prototype.search=function(n){
    this.current = this.root
    return this.find(n)
  }
  
  // ArbolBinarioEncuesta.prototype.posfind=function(x,node){
  //   if(node && node.value.Grado == x){
  //     array.push(node.value);
  //     this.posfind(x,node.left);
  //     this.posfind(x,node.right);
  //   }
  //   return false 
  // }

  ArbolBinarioEncuesta.prototype.find=function(x){
    if(this.current && this.current.value.Grado == x){
      // this.posfind(x,this.current);
      return this.current.repeat.concat(this.current.value); 
    }else if( this.current && this.current.value.Grado<x){
  ...
  ArbolBinarioEncuesta.prototype.hasLeft = function(node){
    return !!node.left
  }
  ArbolBinarioEncuesta.prototype.farestLeft=function(node){
  ...
  ArbolBinarioEncuesta.prototype.delete=function(x){
  ...
  ArbolBinarioEncuesta.prototype.findNodeAndDestroy=function(node){
    this.current = this.root
    while(true){
  ...
  ArbolBinarioEncuesta.prototype.getHeight=function(node){
    node.height=0
    var heightRight=0
    var heightLeft=0
    if(node.left){
  ...
  ArbolBinarioEncuesta.prototype.checkBalance = function(node){
    if(node.left){
